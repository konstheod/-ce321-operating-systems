...
void print_rq () {
	...
	while(curr->next != rq->head) {
		curr = curr->next;
		printf(", %p", curr);
		printf("burst_time: %llu exp_burst: %llu\n",curr->burst, curr->exp_burst);
	};
	printf("\n");
}
...
void schedule()
{
	...
	long long min_exp_burst=0;
	int i = 0;
	
	printf("In schedule\n");
	
	/*prin kalesoume thn context_switch allazoume sthn current->burst ton xrono */
